...
class digital:
    ...
    def Fz(self, T_max = 10, fb=1, plot=False, save=False, title =''):
        ...
        if plot == True:
            ...
            plt.plot(time_plot, mag_plot, linewidth=2, color='blue')
            plt.plot(time_plot, np.ones(len(time_plot)), color='black', alpha=.5, linewidth=2)
            plt.xlabel('Nº de amostras', fontfamily='serif', fontsize='18')
            plt.ylabel('Amplitude', fontfamily='serif', fontsize='18')
            plt.tight_layout()
            plt.grid()
